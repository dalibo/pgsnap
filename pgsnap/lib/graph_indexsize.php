...
if (pg_num_rows($rows) > 0) {
  ...
  while ($row = pg_fetch_array($rows)) {
    ...
    if (strlen($ticks)>0)
      $ticks .= ",";
    if (strlen($indexnames)>0)
      $indexnames .= ",";
    if (strlen($indexcomments)>0)
      $indexcomments .= ",";
    $data .= "[".$i.",".$row['size']."]";
    $ticks .= "[".$i.",'".$row['relname']."']";
    $indexnames .= "'".$row['nspname'].".".$row['relname']."'";
    $indexcomments .= "'".$comments['relations'][$row['nspname']][$row['relname']]."'";
    $i++;
  }
    ...
    graph = Flotr.draw(document.getElementById(\'graphcontainer\'), [ data ],
    {
      ...
      mouse: {
        ...
        trackFormatter: function(point) {
          var indexnames=['.$indexnames.'];
          var indexcomments=['.$indexcomments.'];
          return indexnames[Math.floor(point.x) - 1]
                 + "<br/>"
                 + point.y.toString() + "MB"
                 + "<br/>"
                 + indexcomments[Math.floor(point.x) - 1];
        }
      },
    });
    })();
